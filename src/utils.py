...
def get_xlsx_data_dict(file_name: str) -> list[Any] | str:
...
def show_cards(time_data: str, transactions: list[dict[Any, Any]]) -> list[dict]:
    """Выводит последние 4 цифры карты, общая сумма расходов, кэшбэк"""
    show_cards_list = []
...
def show_top_5_transactions(
    ...
    sorted_transactions = sorted(
        month_transactions,
        key=lambda operation: operation["payment_sum"],
        reverse=True,
    )
    list_index = 1
    top_5_transactions = []
